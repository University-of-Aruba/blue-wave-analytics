...
**4. Prove it from the console.** The Files pane can show you a folder that R is
not pointing at. Run these two lines:
...
The first should end in `blue-wave`. The second should list your files. If it
returns `character(0)`, R is looking somewhere else: use **Session > Set Working
Directory > Choose Directory** and pick the `blue-wave` folder itself, not the
`data` folder inside it. Pointing R at `data` is the most common version of this
mistake, and it breaks every `"data/..."` path in the course by one level.
...
Count the rows. The CSV gave you 94 players and this gives you 48, because this
workbook has the squads split across two sheets and `read_excel()` takes the
first one and says nothing about it. Silence is the danger here: an Excel import
that quietly returns half your cases looks exactly like one that worked.
...
To get the whole dataset, read both and stack them with `bind_rows()`, which
puts one table underneath the other:
...
Ninety-four again. Whenever you import from Excel, check the row count against
what you expect before you analyse anything. `excel_sheets("data/blue_wave_squad.xlsx")`
lists every sheet in a workbook, which is the fastest way to find out what you
were not given.
...
spc_tbl_ [94 × 5] (S3: spec_tbl_df/tbl_df/tbl/data.frame)
 ...
 - attr(*, "spec")=
  ...
  .. )
 - attr(*, "problems")=<pointer: 0x55d801555480> 
```
