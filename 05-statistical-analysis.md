...
Load the data and packages. These lines read the files straight from the web, so
they work whatever happened to your folders yesterday and whatever your working
directory is this morning:
...
If your three files are sitting in a `data` folder inside your working
directory, these lines do the same job:
...
Two commands tell you whether that will work before you run it. `getwd()` prints
where R is looking right now, and `list.files("data")` prints what R can see in
the `data` folder from there. If the second one returns `character(0)`, R is
looking somewhere else. Fix it with **Session > Set Working Directory > Choose
Directory**, or keep the web version above and move on.
...
The fix is to say how much you looked. "We tested
