...
World Cup, unbeaten through CONCACAF qualifying, and the squad that did it is
almost entirely based in European professional football. Very few of those
players play their club football on the island.

That is a striking fact, and it is a data fact before it is a football fact.
Where do the players actually play? At what level? How does the men's squad
compare with the women's, and how does Curaçao compare with Aruba, which has a
similar history and a very different result? Every one of those questions is a
table, a filter, a group summary, and a chart. By Wednesday afternoon you will
be answering them yourself, from a script you wrote.
...
squad |>
  ...
  count(island, tier_group) |>
  ggplot(aes(x = island, y = n, fill = tier_group)) +
...
histories that track each other closely. Completely different answers. Nobody in this room needed a statistician to care about that question.
What you needed was a way to ask it, and you just watched it take twelve lines."
...
2. Copy 103 player names, clubs, and positions into Excel by hand. An hour if
   you are quick and do not lose concentration.
3. Discover that club names are spelled three different ways, that some players
   have no club listed, and that Jong Holland is a Curaçao club despite the
   name. Clean by hand.
4. Import into SPSS. Recode league level into a grouping variable, because the
